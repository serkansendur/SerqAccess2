...
namespace SerqAccess.EasyData.EXCEL
{
    public class ExcelWriter
    {
        public static void Write(List<System.Data.DataTable> workSheets, string filePath)
        {
        ...
        public static void WriteClosed(List<System.Data.DataTable> workSheets, string filePath)
        {
            using (XLWorkbook wb = new XLWorkbook())
            {
                int counter = 0;
                foreach (System.Data.DataTable worksheet in workSheets)
                {
                    counter++;
                    string workSheetName = !string.IsNullOrEmpty(worksheet.TableName) ? worksheet.TableName : "Sheet" + counter;
                    wb.Worksheets.Add(worksheet, workSheetName);
                }
                wb.SaveAs(filePath);
            }
        }
